...
namespace
{
    void dwarf_runtime_error(Dwarf_Error err, Dwarf_Ptr arg) {
        ...
        msg.append((dwarf_errno(err) == DW_DLA_ERROR) ? dwarf_errmsg(err) : "unspecified error");
...
{
    Dwarf_Error err;
    if (dwarf_elf_init(elf, DW_DLC_READ, &dwarf_runtime_error, nullptr, &this->dbg, &err) != DW_DLV_OK) {
        this->dbg = nullptr;
    }
}
...
void dwarf_t::subroutines(subroutine_map& table)
{
    ...
    while (dwarf_next_cu_header_c(this->dbg, 1, // is_info
...
template<typename T>
void dwarf_t::siblings(die_ptr die, void (dwarf_t::*fn)(Dwarf_Die, T &), T &arg)
{
    for (;;) {
        Dwarf_Die p = die.get();
...
void dwarf_t::die_subprogram(Dwarf_Die die, subroutine_map& table)
{
    ...
    if ((name == nullptr) ||
        (dwarf_lowpc(die, &lowpc, nullptr) != DW_DLV_OK) ||
        (dwarf_highpc_b(die, &highpc, &form, &form_class, nullptr) != DW_DLV_OK)) {
        return;
    }
    ...
    bool placed;
    std::tie(iter, placed) = table.emplace(lowpc, subroutine_t(name.get(), highpc, true));
    if (!placed) {
        return;
    }
    ...
    if (dwarf_child(die, &die, nullptr) == DW_DLV_OK) {
        die_ptr die_wrap(die, dwarf_deleter(dbg));
        this->siblings(std::move(die_wrap), &dwarf_t::die_callsite, iter->second.callsites);
    }
}
